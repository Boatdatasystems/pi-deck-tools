...
def create_planner_route(
    ...
    Uses the known navobj.db schema:
      routes(guid PK NOT NULL, name, ...)
      routepoints(guid PK NOT NULL, lat, lon, Name, ...)
      routepoints_link(route_guid, point_guid, point_order)
    ...
    try:
        ...
        # Remove any existing planner route with this name.
        existing = con.execute(
            "SELECT guid FROM routes WHERE UPPER(name) = UPPER(?)",
            (planner_route_name,),
        ).fetchall()
        for (existing_guid,) in existing:
            con.execute(
                "DELETE FROM routepoints_link WHERE route_guid = ?", (existing_guid,)
            )
            con.execute("DELETE FROM routes WHERE guid = ?", (existing_guid,))

        # Insert the route row.
        route_guid = str(uuid.uuid4())
        con.execute(
            "INSERT INTO routes (guid, name) VALUES (?, ?)",
            (route_guid, planner_route_name),
        )

        # Insert each waypoint and link it to the route.
        for idx, point in enumerate(timeline_points, start=1):
            wp_guid = str(uuid.uuid4())
            wp_name = str(point.get("name") or f"Planner {idx}")
            lat = float(point["lat"])
            lon = float(point["lon"])

            con.execute(
                "INSERT INTO routepoints (guid, lat, lon, Name) VALUES (?, ?, ?, ?)",
                (wp_guid, lat, lon, wp_name),
            )
            con.execute(
                "INSERT INTO routepoints_link (route_guid, point_guid, point_order)"
                " VALUES (?, ?, ?)",
                (route_guid, wp_guid, idx),
            )
